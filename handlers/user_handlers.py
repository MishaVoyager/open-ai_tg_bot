...
async def search_text_handler(message: Message, visitor: Visitor) -> None:
    ...
    result = generate_text(str(message.from_user.id), message.text, visitor.model)
...
async def search_audio_handler(message: Message, visitor: Visitor) -> None:
    ...
    tmp_message = await message.answer(get_random_processing_phrase())
    result = generate_text(str(message.from_user.id), transcript, visitor.model)
    await tmp_message.delete()
    text = result.refusal if result.refusal else result.content
    await send_text_any_size(message, text)
...
async def continue_friend_chat_audio_handler(message: Message, visitor: Visitor) -> None:
    ...
    transcript = audio_to_text(in_memory_file)
    await message.answer(f"Транскрипт вашего аудио: \n\n{transcript}")
    result = get_answer_from_friend(str(message.from_user.id), transcript, visitor.model)
    if result.refusal:
...
async def continue_friend_chat_text_handler(message: Message, visitor: Visitor) -> None:
    result = get_answer_from_friend(str(message.from_user.id), message.text, visitor.model)
    text = result.refusal if result.refusal else result.content
    await send_text_any_size(message, text)
...
async def feedback_audio_handler(message: Message, visitor: Visitor) -> None:
    ...
    transcript = audio_to_text(in_memory_file)
    await message.answer(f"Транскрипт вашего аудио: \n\n{transcript}")
    result = get_english_teacher_comment(str(message.from_user.id), transcript, visitor.model)
...
async def feedback_text_handler(message: Message, visitor: Visitor) -> None:
    result = get_english_teacher_comment(str(message.from_user.id), message.text, visitor.model)
    text = result.refusal if result.refusal else f"Коммент учителя английского: \n\n{result.content}"
    await send_text_any_size(message, text)
